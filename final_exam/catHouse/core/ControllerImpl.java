...
public class ControllerImpl implements Controller {
    ...
    public String sumOfAll(String houseName) {
        House house = houses.stream().filter(h -> h.getName().equals(houseName)).findFirst().orElse(null);
        assert house != null;
        double catsPrices = house.getCats().stream().mapToDouble(Cat::getPrice).sum();
        double toysPrice = house.getToys().stream().mapToDouble(Toy::getPrice).sum();
        double housePrice = catsPrices + toysPrice;

        return String.format(VALUE_HOUSE, houseName, housePrice);
    }
